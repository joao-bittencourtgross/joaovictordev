<?PHP
class DiferencialEmpController extends EmprendimentosAppController {
    ...
    public function admin_index($id_emp = null){
        ...
        if($id_emp){
            $this->paginate['conditions'] = array('DiferencialEmp.empreendimento_id'=>$id_emp);
            $value_select = $id_emp;
        }
        $this->paginate['order'] = array('DiferencialEmp.order_registro'=>'ASC','DiferencialEmp.created'=>'DESC');
        $posts = $this->paginate('DiferencialEmp');
    ...
    public function admin_add(){
        ...
        if($this->request->data && ($this->request->is('post') || $this->request->is('put'))){
            if($this->DiferencialEmp->save($this->request->data)){
    ...
    public function admin_edit($id){
        ...
        $registro = $this->DiferencialEmp->read('*',$id); 
        $this->data = $registro; 
    }
    
    public function admin_delete($id){
        $this->autoRender = false;
        if($this->DiferencialEmp->delete($id)){
            $this->redirect(array('action'=>'index'));
        }
    }   
    ...
    public function admin_order($id,$order){
        $this->autoRender = false;
        $this->DiferencialEmp->query('UPDATE tb_emp_diferenciais SET order_registro = "'.$order.'" WHERE id = "'.$id.'";');
    }
