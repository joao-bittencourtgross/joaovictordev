<?PHP
class ProgressoEmpController extends EmprendimentosAppController {
    ...
    public function admin_index($id_emp = null){
        ...
        if($id_emp){
            $this->paginate['conditions'] = array('ProgressoEmp.empreendimento_id'=>$id_emp);
            $value_select = $id_emp;
        }
        $this->paginate['order'] = array('ProgressoEmp.order_registro'=>'ASC','ProgressoEmp.created'=>'DESC');
        $posts = $this->paginate('ProgressoEmp');
    ...
    public function admin_add(){
        ...
        if($this->request->data && ($this->request->is('post') || $this->request->is('put'))){
            if($this->ProgressoEmp->save($this->request->data)){
    ...
    public function admin_edit($id){
        ...
        $registro = $this->ProgressoEmp->read('*',$id); 
        $this->data = $registro; 
    }
    
    public function admin_delete($id){
        $this->autoRender = false;
        if($this->ProgressoEmp->delete($id)){
            $this->redirect(array('action'=>'index'));
        }
    }   
    ...
    public function admin_order($id,$order){
        $this->autoRender = false;
        $this->ProgressoEmp->query('UPDATE tb_emp_progresso_obra SET order_registro = "'.$order.'" WHERE id = "'.$id.'";');
    }
