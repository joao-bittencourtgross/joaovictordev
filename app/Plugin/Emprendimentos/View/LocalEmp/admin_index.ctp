        ...
        <div class="table-responsive">
            <table class="mb-0 table">
                ...
                <tbody class="sortable">
                    ...
                    ?>
                        <tr data-id="<?PHP echo $post['LocalEmp']['id']; ?>">
                            <td><?PHP echo $post['LocalEmp']['title']?></td>
                            <td><?PHP echo $post['Emprendimento']['title']?></td>
                            <td><?PHP echo $this->Locker->link('Editar',array('action'=>'admin_edit',$post['LocalEmp']['id']),array('class'=>'btn-edit'))?></td>
                            <td><?PHP echo $this->Locker->link('Excluir',array('action'=>'admin_delete',$post['LocalEmp']['id']),array('class'=>'btn-remove'),'Tem certeza?')?></td>
                        </tr>
                    <?PHP 
                        } 
                    ?>
...
<script>
        ...
        $(".sortable").sortable({
            stop:function(evt,ui){
                $(".sortable>tr").each(function(i,e){
                    ...
                    var index=$(e).index();
                    $.getJSON(base+'/admin/emprendimentos/local_emp/order/'+id+'/'+index,function(data){
                    });
                });
            }
        });
    ...
    $('#select_emps').change(function(){
        window.location.href = base+'/admin/emprendimentos/local_emp/index/'+this.value;;
    });
</script>