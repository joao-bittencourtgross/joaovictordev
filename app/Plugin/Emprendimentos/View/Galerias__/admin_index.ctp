                ...
                <tbody class="sortable">
                    ...
                    ?>
                        <tr data-id="<?PHP echo $post['Galeria']['id']; ?>">
                            ...
                            <td><?PHP echo $this->Html->image('painel/ativo_'.$post['Galeria']['ativo'].'.png',array('alt'=>'')); ?></td>
                            <td><?PHP echo $this->Locker->link('Editar',array('action'=>'admin_edit',$post['Galeria']['id']),array('class'=>'btn-edit'))?></td>
                            <td><?PHP echo $this->Locker->link('Excluir',array('action'=>'admin_delete',$post['Galeria']['id']),array('class'=>'btn-remove'),'Tem certeza?')?></td>
                        </tr>
                    <?PHP 
                        } 
                    ?>
...
<script>
        ...
        $(".sortable").sortable({
            stop:function(evt,ui){
                $(".sortable>tr").each(function(i,e){
                    ...
                    var index=$(e).index();
                    $.getJSON(base+'/admin/emprendimentos/galerias/order/'+id+'/'+index,function(data){
                    });
                });
            }
        });
    ...
    $('#select_emps').change(function(){
        window.location.href = base+'/admin/emprendimentos/galerias/index/'+this.value;;
    });
</script>