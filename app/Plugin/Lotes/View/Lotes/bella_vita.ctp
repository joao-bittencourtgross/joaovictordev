...
<h1 class="htl">Conheça o mais novo loteamento de Cascavel <br> Bella Vita!</h1>
...
<div class="mapa-lotes">
    ...
    <img src="<?PHP echo $this->Html->url('/img/mapa/lotes_bella_vita.png',true); ?>" usemap="#map_lotes" id="mapal" border="0">
</div>
...
//            echo '<area name="'.$lote['Lote']['slug'].'" class="open-vl" href="'.$this->Html->url('/detalhe-lote/'.$lote['Lote']['slug'],true).'" title="'.$lote['Lote']['title'].'" coords="'.$lote['Lote']['cordenadas'].'" shape="circle">';
            echo '<area name="'.$lote['Lote']['slug'].'" class="open-vl" href="'.$this->Html->url('/detalhe-lote-bella-vita/'.$lote['Lote']['slug'],true).'" title="'.$lote['Lote']['title'].'" coords="'.$lote['Lote']['cordenadas'].'" shape="circle">';
        }
    ?>
</map>
...
<footer>
    ...
    <div class="box-whatsapp">
        <?PHP 
            echo $this->Html->link('<span>Agendar visita</span> '.$this->Html->image('button_whatsapp.png',array('alt'=>'Fale conosco via WhatsApp','title'=>'Fale conosco via WhatsApp')),'https://api.whatsapp.com/send?phone=554599883077&text=Olá, gostaria de mais informações sobre o Loteamento Bella Vita',array('escape'=>false,'target'=>'_blank'));
        ?>
    </div>
</footer>
...
//                             window.location = document.URL.replace('?modal=open','');
                             window.location.href = base+'/mapa/loteamento-bella-vita';
                        },
                        success: function() {
