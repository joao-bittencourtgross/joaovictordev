        ...
        <div class="table-responsive">
            <table class="mb-0 table">
                ...
                <tbody class="sortable">
                    ...
                    ?>
                        <tr data-id="<?PHP echo $post['CategoriaLote']['id']; ?>">
                            <td><?PHP echo $post['CategoriaLote']['title']?></td>
                            <td><?PHP echo $this->Locker->link('Editar',array('action'=>'admin_edit',$post['CategoriaLote']['id']),array('class'=>'btn-edit'))?></td>
                            <td><?PHP echo $this->Locker->link('Excluir',array('action'=>'admin_delete',$post['CategoriaLote']['id']),array('class'=>'btn-remove'),'Tem certeza?')?></td>
                        </tr>
                    <?PHP 
                        } 
                    ?>
        ...
        $(".sortable").sortable({
            stop:function(evt,ui){
                $(".sortable>tr").each(function(i,e){
                    ...
                    var index=$(e).index();
                    $.getJSON(base+'/admin/lotes/categoria_lotes/order/'+id+'/'+index,function(data){
                    });
                });
            }
        });
