...
class LockerHelper extends AppHelper{
    ...
    private function check($url,$output){
        ...
        if($this->base!='/') {
            $url = Router::parse(str_replace($this->base,'',$url));
        }else {
        ...
        ######### ACTIONS ################
        if(isset($this->config->sectors)){ 
            $actions = $this->config->sectors;
        }
        ...
        if(isset($actions[$controller])){
            ...
            if(isset($actions[$controller]['*'])){
                ...
                if(!in_array($group,$actions[$controller]['*'])){
                    return null;
                } 
            }
            if(isset($actions[$controller][$action])){
                ...
                if(!in_array($group,$actions[$controller][$action])){
                    return null;
                } 
            }
        }
